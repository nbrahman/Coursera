...
Sampling Data Set is created by choosing 1% rows (considering the Data Set volumns and the computation power of my machine) of the each and every actual data set. This sampling percentage can be changed by changing `samplingPercentage` parameter.
```{r, include=TRUE, message=FALSE, warning=FALSE}
set.seed(5000) # setting the random seed for reproducibility
samplingPercentage = 0.5 # sampling percentage, can be changed depending up on the results
...
title ("Word Cloud - US English Combined")
```
